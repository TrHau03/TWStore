      ...
      <View>
        {data.map((item: Card) =>
          <LinearGradient start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} colors={['#46caf3', '#5cbae3', '#68b1d9']} style={styles.itemCardBank} key={item.id}>
            ...
            <View>
              <PatternFormat displayType='text' value={item.numberCard} format="#### #### #### ####" renderText={(numberCard: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | null | undefined) => <Text style={styles.numberCard}>{numberCard}</Text>} />
            </View>
