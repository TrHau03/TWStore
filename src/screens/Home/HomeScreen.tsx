...
import {
  ...
  RootStackScreenEnumHome,
} from '../../component/Root/RootStackHome';
...
import {
  RootStackParamListExplore,
  RootStackScreenEnumExplore,
} from '../../component/Root/RootStackExplore';
...
import {
  ...
  searchProduct,
} from '../../redux/silces/HomeSelector';
...
const HomeScreen = ({navigation}: NativeStackHeaderProps) => {
  ...
  const [textInputSearchOnChange, setTextInputSearchOnChange] =
    useState<string>();
  ...
  const renderItem = ({
    ...
    item: {_id: string; name: string; linkIcon: string};
  }) => (
    <Pressable
      style={styles.item}
      onPress={() =>
        navigation.navigate('Explore', {
          screen: RootStackScreenEnumExplore.Category_Detail_Screen,
          params: {brandID: item._id},
        })
      }>
  ...
  return (
    <SafeAreaView
      ...
      <View style={styles.top}>
        <View
          style={
            !textInputStatus
              ? styles.headerLeft
              : [styles.headerLeft, {borderColor: COLORS.gray}]
          }>
          ...
          {textInputStatus && (
            <Pressable
              ...
              style={{position: 'absolute', right: 10, borderRadius: 5}}>
        ...
        <View style={styles.headerRight}>
          <TouchableOpacity
            onPress={() =>
              navigation.navigate(RootStackScreenEnumHome.NotificationScreen)
            }>
      ...
      {textInputStatus ? (
        <FlatList
          data={productFilter.filter((item: any) =>
            item.productName
              .toLowerCase()
              .includes(textInputSearch?.toLowerCase()),
          )}
        ...
        <ScrollView
                ...
                <Pressable
                  onPress={() =>
                    navigation.navigate('Offer', {
                      screen: RootStackScreenEnumOffer.OfferHome,
                    })
                  }
                  key={e._id}>
                  <Image
          ...
          <View>
            <Image
              style={styles.imgrecomended}
              source={require('../../asset/image/bannerRecomendedProduct.png')}
            />
          </View>
          <FlatList
            scrollEnabled={false}
